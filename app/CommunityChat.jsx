...
export default function CommunityChat({ user }) {
  ...
  const [guestName, setGuestName] = useState(() => {
    try { return localStorage.getItem('chat_guest_name') || ''; } catch { return ''; }
  });
  ...
  useEffect(() => {
    const subs = CHANNELS.filter(c => c.id !== channel).map(c => {
      return sb.channel(`chat_unread:${c.id}`)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages', filter: `channel=eq.${c.id}` },
  ...
  const send = useCallback(async () => {
    ...
    await sb.from('chat_messages').insert({
      ...
      author_id: user?.id || `guest_${displayName}`,
      content: text,
    }).catch(() => {});
