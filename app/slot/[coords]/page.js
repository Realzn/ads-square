...
export async function generateMetadata({ params }) {
  ...
  // Image OG dynamique — générée par /api/og pour chaque slot
  const ogImageUrl = `${APP_URL}/api/og?x=${x}&y=${y}`;

  // Fetch slot data server-side for metadata text
  try {
      ...
      return {
        ...
        openGraph: {
          ...
          images: [{
            ...
            width: 1200,
            height: 630,
            alt: `${booking.display_name} · Slot [${x},${y}] ${tier.toUpperCase()} · AdsMostFair`,
          }],
        },
        twitter: {
          card: 'summary_large_image',
          title: `${booking.display_name} · AdsMostFair`,
          description: booking.slogan || `Slot ${tier.toUpperCase()} [${x},${y}]`,
          images: [ogImageUrl],
        },
      };
    }
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{
        ...
        width: 1200,
        height: 630,
        alt: `Slot [${x},${y}] disponible sur AdsMostFair`,
      }],
    },
    twitter: {
      card: 'summary_large_image',
