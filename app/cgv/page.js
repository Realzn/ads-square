...
export default function CGVPage() {
  ...
  const sections = [
    { sectionTitle: t('cgv_s1_title'), content: t('cgv_s1') },
    { sectionTitle: t('cgv_s2_title'), content: t('cgv_s2') },
    { sectionTitle: t('cgv_s3_title'), content: t('cgv_s3') },
    { sectionTitle: t('cgv_s4_title'), content: t('cgv_s4') },
    { sectionTitle: t('cgv_s5_title'), content: t('cgv_s5') },
    { sectionTitle: t('cgv_s6_title'), content: t('cgv_s6') },
    { sectionTitle: t('cgv_s7_title'), content: t('cgv_s7') },
  ];
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: "'Rajdhani','Sora',system-ui,sans-serif", color: C.text }}>
      ...
      <nav style={{ borderBottom: `1px solid ${C.border}`, padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(0,2,10,0.98)', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ color: C.muted, fontSize: 12 }}>/ {t('nav_terms')}</span>
        </div>
        <LanguageSwitcher compact />
      </nav>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '48px 20px' }}>
        <h1 style={{ fontSize: 36, fontWeight: 900, margin: '0 0 10px' }}>{t('cgv_title')}</h1>
        <p style={{ fontSize: 14, color: C.muted, margin: '0 0 40px' }}>{t('cgv_updated')} : {new Date().toLocaleDateString()}</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          {sections.map(({ sectionTitle, content }, i) => (
            <div key={i} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: '22px 24px' }}>
              <h2 style={{ fontSize: 18, fontWeight: 800, color: C.cyan, margin: '0 0 10px' }}>{i + 1}. {sectionTitle}</h2>
