...
/**
 * Upload une image de bloc vers Supabase Storage.
 * Retourne l'URL publique de l'image.
 * Nécessite un bucket public "block-images" dans Supabase Storage.
 */
export async function uploadBlockImage(file) {
  if (!supabase) throw new Error('Supabase non configuré');

  const ext  = file.name.split('.').pop().toLowerCase() || 'jpg';
  const name = `block-${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
