 ...
 * ⚠️ Passe par /api/bookings/update (service_role) car la RLS bloque
 *    tout UPDATE direct depuis le client anon, même pour le propriétaire.
 */
export async function updateBookingContent(bookingId, updates) {
  if (!supabase) throw new Error('Supabase not configured');

  // Récupérer le token de session pour prouver l'identité côté serveur
  const session = await getSession();
  ...
  if (!res.ok) throw new Error(json.error || 'Mise à jour échouée');
}

/**
 * Mettre à jour le profil advertiser
 * ⚠️ Passe par /api/bookings/update (service_role) — RLS bloque le client anon
 */
export async function updateAdvertiserProfile(advertiserId, updates) {
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(json.error || 'Mise à jour profil échouée');
}
 ...
 * ⚠️ Passe par /api/bookings/update — RLS bloque le client anon
 */
export async function toggleBookingBoost(bookingId, isBoosted) {
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(json.error || 'Boost échoué');
}