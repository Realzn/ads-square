// lib/emails.js — Templates transactionnels AdsMostFair · DA Dyson Cosmos
...
async function send({ to, subject, html }) {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: process.env.EMAIL_FROM || 'AdsMostFair <noreply@adsmostfair.com>',
      to,
      subject,
      html,
    }),
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:540px;">
    ...
    <tr><td style="padding-top:24px;">
      ...
      <p style="font-family:'JetBrains Mono',monospace;font-size:8px;letter-spacing:.12em;color:rgba(60,100,150,0.45);line-height:1.9;text-align:center;">
        AdsMostFair (société en cours de création) · Les Ulis 91940, France<br>
...
export async function sendExpiryReminder({ to, bookingDetails, daysLeft, displayName: dn, tier: t, slotX: sx, slotY: sy, expiresAt: ea }) {
  ...
  return send({
    to,
    subject: `${urgent ? '⚠ URGENT — ' : ''}Votre bloc expire dans ${daysLeft} jour${daysLeft > 1 ? 's' : ''} — AdsMostFair`,
    html: base({
...
export async function sendExpiryNotification({ to, bookingDetails, displayName: dn, tier: t, slotX: sx, slotY: sy }) {
  ...
  return send({
    to,
    subject: `Votre bloc (${slotX},${slotY}) a expiré — AdsMostFair`,
    html: base({
      ...
      body: `
        ...
        <div style="margin-top:28px;padding:14px 16px;
          ...
          ${subText(`Merci d'avoir fait confiance à AdsMostFair. Votre historique et vos contenus sont conservés dans votre dashboard.`)}
        </div>
      `,
    }),
  });
...
export async function sendWaitlistConfirmation({ to, lang }) {
  const site = SITE_URL();
  const isEn = lang === 'en';
  return send({
    to,
    subject: isEn
      ? '◈ You are on the list — AdsMostFair'
      : '◈ Vous êtes sur la liste — AdsMostFair',
    html: base({
      preheader: isEn
        ? 'Your place on the AdsMostFair waitlist is confirmed. We\'ll notify you as soon as it opens.'
        : 'Votre place sur la liste d\'attente AdsMostFair est confirmée. On vous prévient dès l\'ouverture.',
      accentColor: '#E8A020',
      body: `
        <div style="text-align:center;margin-bottom:24px;">
          <div style="display:inline-block;width:56px;height:56px;
        ...
        ${sectionTitle(isEn ? 'You are on the list.' : 'Vous êtes dans la liste.', '#E8A020')}
        <div style="margin-top:12px;margin-bottom:24px;">
          ${subText(isEn
            ? `Your email has been registered for priority access to AdsMostFair.<br><br>
              As soon as the public beta opens, you will be the first to be notified.<br>
              In the meantime, you can explore the Dyson Sphere in read-only mode.`
            : `Votre email a bien été enregistré pour l'accès prioritaire à AdsMostFair.<br><br>
        ...
        ${ctaButton(
          isEn ? 'EXPLORE THE SPHERE' : 'EXPLORER LA SPHÈRE',
          site,
          '#E8A020'
        )}
      `,
...
export async function sendOfferNotification({ to, offerDetails }) {
  ...
  return send({
    to,
    subject: `◈ Nouvelle offre de rachat sur votre bloc (${slotX},${slotY}) — AdsMostFair`,
    html: base({
...
// ─── 6. Notification admin — nouvelle inscription waitlist ────────────────────
export async function sendWaitlistAdminNotification({ userEmail, profile }) {
  return send({
    to: 'waitlist@adsmostfair.com',
    subject: `◈ Nouvelle inscription waitlist — ${userEmail}`,
    html: base({
      preheader: `${userEmail} vient de rejoindre la liste d'attente AdsMostFair.`,
      accentColor: '#E8A020',
      body: `
        ${sectionTitle('Nouvelle inscription waitlist', '#E8A020')}
        <div style="margin-top:12px;margin-bottom:24px;">
          <table width="100%" cellpadding="0" cellspacing="0"
            style="background:rgba(0,200,240,0.03);border:0.5px solid rgba(0,200,240,0.10);">
            <tr><td style="padding:16px 18px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                ${infoRow('EMAIL', userEmail, '#E8A020')}
                ${infoRow('PROFIL', profile || 'Non renseigné', '#DDE6F2')}
                ${infoRow('DATE', new Date().toLocaleString('fr-FR', { timeZone: 'Europe/Paris' }), 'rgba(0,200,240,0.60)')}
              </table>
            </td></tr>
          </table>
        </div>
        ${ctaButton('VOIR LA WAITLIST — ADMIN', (process.env.NEXT_PUBLIC_SITE_URL || 'https://adsmostfair.com') + '/admin', '#E8A020')}
      `,
    }),
  });
}
...
// ─── 7. Bug report ────────────────────────────────────────────────────────────
export async function sendBugReport({ email, description, page, lang }) {
  const isEn = lang === 'en';
  return send({
    to: 'support@adsmostfair.com',
    subject: `🐛 Bug report — ${email}`,
    html: base({
      preheader: `Bug signalé par ${email}${page ? ` sur ${page}` : ''}.`,
      accentColor: '#D02848',
      body: `
        <div style="text-align:center;margin-bottom:24px;">
          <div style="display:inline-block;width:56px;height:56px;
            background:rgba(208,40,72,0.12);border:1px solid rgba(208,40,72,0.44);
            line-height:56px;text-align:center;font-size:28px;">🐛</div>
        </div>

        ${sectionTitle(isEn ? 'Bug Report Received' : 'Rapport de bug reçu', '#D02848')}
        <div style="margin-top:12px;margin-bottom:24px;">
          <table width="100%" cellpadding="0" cellspacing="0"
            style="background:rgba(0,200,240,0.03);border:0.5px solid rgba(0,200,240,0.10);">
            <tr><td style="padding:16px 18px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                ${infoRow(isEn ? 'FROM' : 'DE', email, '#DDE6F2')}
                ${page ? infoRow(isEn ? 'PAGE' : 'PAGE', page, 'rgba(0,200,240,0.60)') : ''}
                ${infoRow(isEn ? 'LANGUAGE' : 'LANGUE', lang === 'en' ? 'English' : 'Français', 'rgba(0,200,240,0.60)')}
                ${infoRow(isEn ? 'DATE' : 'DATE', new Date().toLocaleString('fr-FR', { timeZone: 'Europe/Paris' }), 'rgba(0,200,240,0.60)')}
              </table>
            </td></tr>
          </table>

          <div style="margin-top:16px;padding:16px 18px;background:rgba(208,40,72,0.04);border:0.5px solid rgba(208,40,72,0.18);">
            <div style="font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:.12em;color:rgba(208,40,72,0.70);margin-bottom:8px;">
              ${isEn ? 'DESCRIPTION' : 'DESCRIPTION'}
            </div>
            <div style="font-family:'Rajdhani',sans-serif;font-size:14px;color:#DDE6F2;line-height:1.7;white-space:pre-wrap;">${description.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
          </div>
        </div>
      `,
    }),
...
// ─── 8. Email de lancement — envoyé à tous les inscrits waitlist ──────────────
export async function sendLaunchEmail({ to, lang }) {
  const site = process.env.NEXT_PUBLIC_SITE_URL || 'https://adsmostfair.com';
  const isEn = lang === 'en';

  return send({
    to,
    subject: isEn
      ? '◈ AdsMostFair is now open — your spot is waiting'
      : '◈ AdsMostFair est maintenant ouvert — votre place vous attend',
    html: base({
      preheader: isEn
        ? 'The AdsMostFair grid is now open. Book your advertising block today!'
        : 'La grille AdsMostFair est maintenant ouverte. Réservez votre bloc publicitaire dès aujourd\'hui !',
      accentColor: '#E8A020',
      body: `
        <div style="text-align:center;margin-bottom:28px;">
          <div style="display:inline-block;width:72px;height:72px;
            background:rgba(232,160,32,0.12);border:1px solid rgba(232,160,32,0.44);
            line-height:72px;text-align:center;font-size:32px;color:#E8A020;">◈</div>
        </div>

        ${sectionTitle(isEn ? 'The platform is OPEN.' : 'La plateforme est OUVERTE.', '#E8A020')}

        <div style="margin-top:16px;margin-bottom:28px;">
          ${subText(isEn
            ? `You signed up for early access — the moment has arrived.<br><br>
              AdsMostFair is now open to all advertisers. <strong style="color:#E8A020;">Reserve your block on the Dyson Sphere</strong> directly from the 3D grid, from €1/day, with no agency and no minimum budget.<br><br>
              As a priority member of our waitlist, you benefit from our <strong style="color:#E8A020;">launch pricing</strong> — the best rates we'll ever offer.`
            : `Vous vous étiez inscrit·e pour un accès prioritaire — ce moment est arrivé.<br><br>
              AdsMostFair est maintenant ouvert à tous les annonceurs. <strong style="color:#E8A020;">Réservez votre bloc sur la Sphère de Dyson</strong> directement depuis la grille 3D, dès 1€/jour, sans agence et sans budget minimum.<br><br>
              En tant que membre prioritaire de notre liste d'attente, vous bénéficiez de nos <strong style="color:#E8A020;">tarifs de lancement</strong> — les meilleurs que nous proposerons jamais.`
          )}
        </div>

        <table width="100%" cellpadding="0" cellspacing="0"
          style="background:rgba(232,160,32,0.04);border:0.5px solid rgba(232,160,32,0.22);margin-bottom:28px;">
          <tr><td style="padding:20px 18px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              ${infoRow(isEn ? 'FROM' : 'À PARTIR DE', isEn ? '€1/day' : '1€/jour', '#E8A020')}
              ${infoRow(isEn ? 'BLOCKS AVAILABLE' : 'BLOCS DISPONIBLES', '1 296', '#DDE6F2')}
              ${infoRow(isEn ? 'SETUP TIME' : 'MISE EN LIGNE', isEn ? 'Instant' : 'Immédiate', '#00D880')}
              ${infoRow(isEn ? 'MINIMUM BUDGET' : 'BUDGET MINIMUM', isEn ? 'None' : 'Aucun', '#00D880')}
            </table>
          </td></tr>
        </table>

        ${ctaButton(
          isEn ? 'EXPLORE THE GRID & BOOK NOW' : 'EXPLORER LA GRILLE & RÉSERVER',
          site,
          '#E8A020'
        )}
        ...
        <div style="margin-top:20px;text-align:center;">
          ${subText(isEn
            ? 'No agency · No minimum budget · Instant results'
            : 'Sans agence · Sans budget minimum · Résultat immédiat'
          )}
        </div>
      `,
    }),
