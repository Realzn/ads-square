...
// ─────────────────────────────────────────────────────────────
// EMAIL 5 — Confirmation d'inscription waitlist
// ─────────────────────────────────────────────────────────────
export async function sendWaitlistConfirmation({ to }) {
  const siteUrl = 'https://ads-square.pages.dev';

  const html = baseLayout(`
    ${accentBar(D.accent)}
    <div style="padding:32px 32px 0;">
      <div style="font-size:32px;margin-bottom:10px;">✦</div>
      <h1 style="margin:0 0 8px;font-size:22px;font-weight:800;color:${D.text};letter-spacing:-0.02em;">Vous êtes sur la liste !</h1>
      <p style="margin:0 0 28px;font-size:14px;color:${D.muted};line-height:1.7;">
        Merci pour votre intérêt. Vous serez parmi les premiers à accéder à ADS-SQUARE
        dès l'ouverture de nouveaux blocs.<br/>
        Nous vous contacterons à cette adresse dès que votre accès est disponible.
      </p>

      <!-- Ce qui vous attend -->
      <p style="margin:0 0 12px;font-size:10px;color:${D.muted};text-transform:uppercase;letter-spacing:0.08em;">CE QUI VOUS ATTEND</p>
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
        ${[
          ['✦', 'Un bloc dédié sur la grille', 'Votre espace publicitaire visible par tous les visiteurs en temps réel.', D.accent],
          ['⚡', 'Boost de visibilité', 'Passez en avant de la grille et dans le ticker en haut de page.', '#f0b429'],
          ['📊', 'Stats en direct', 'Vues, clics et CTR mis à jour en temps réel dans votre espace.', D.cyan],
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="32" style="vertical-align:top;padding-top:2px;">
                    <div style="width:24px;height:24px;border-radius:6px;background:${col}15;border:1px solid ${col}30;text-align:center;line-height:24px;font-size:11px;">${icon}</div>
                  </td>
                  <td style="padding-left:10px;">
                    <div style="font-size:13px;font-weight:700;color:${D.text};margin-bottom:2px;">${title}</div>
                    <div style="font-size:12px;color:${D.muted};line-height:1.5;">${desc}</div>
                  </td>
                </tr>
              </table>
            </td>
      ...
      <!-- Explorer la grille -->
      <div style="text-align:center;padding-bottom:32px;">
        <p style="margin:0 0 16px;font-size:13px;color:${D.muted};">En attendant, explorez la grille publique :</p>
        ${ctaButton(siteUrl, 'Voir la grille →', D.accent)}
  ...
  return sendEmail({ to, subject: `✦ Vous êtes sur la liste — ADS-SQUARE`, html });
}
